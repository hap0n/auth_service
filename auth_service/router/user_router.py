...
@user_router.post("/users/", response_model=schemas.user.User, tags=["users"])
...
@user_router.get("/users/", response_model=List[schemas.user.User], tags=["users"])
async def read_users(offset: int, limit: int):
    return UserRepository.get_users(offset=offset, limit=limit)


@user_router.get("/users/{username}", response_model=schemas.user.User, tags=["users"])
